...
Disect is a command line tool to find the dependencies that caused a regression in a python environment by leveraging combinatorical group testing to minimize the number of environments to test.
...
It works similar to 'git bisect', and will repeatedly ask the user to mark dependency sets ad good or bad,
splitting the search space based on the results.

To run it you need to provide:
    1. A pip requirements file where the dependencies that might have caused the regression are pinned to known good versions.
